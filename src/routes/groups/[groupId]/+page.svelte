	...
	{:else}
		<div class="space-y-2">
			{#each activeExpenses as expense}
				...
				<div
					class="flex cursor-pointer items-center justify-between rounded-lg border border-gray-200 bg-white p-4 hover:bg-gray-50"
			...
			<div class="mt-6">
				...
				<div class="space-y-2">
					{#each settledExpenses as expense}
						<div
							class="flex items-center justify-between rounded-lg border border-gray-100 bg-gray-50 p-4"
						>
							<div class="flex-1 opacity-60">
								<div class="font-medium text-gray-500">{expense.description}</div>
							...
							<div class="flex items-center gap-2">
								<span class="font-semibold text-gray-400 opacity-60">
									{formatCurrency(expense.amount, group.currency)}
								</span>
								<form
									method="POST"
								...
								>
									...
									<button
										...
										class="rounded p-1 text-green-500 hover:bg-gray-100 hover:text-gray-400"
										title="Unsettle"
									>
										<svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
											<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" />
...
{#if settlingExpense}
	...
	>
		<div class="w-full max-w-sm rounded-lg bg-white p-6 shadow-xl">
			<h3 class="text-lg font-semibold text-gray-900">Settle expense?</h3>
			{#if share !== null}
