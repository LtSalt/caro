...
<div class="mx-auto max-w-sm py-12">
	...
	{#if form?.success}
		<div class="rounded-lg bg-green-50 p-4 text-sm text-green-800 dark:bg-green-900/30 dark:text-green-300">
			...
			<p class="mt-1">We sent a verification link to <strong>{form.email}</strong>. Please verify your email before signing in.</p>
			<a href="/login" class="mt-3 inline-block font-medium text-green-900 underline dark:text-green-200">Go to sign in</a>
		</div>
	{:else}
		{#if form?.error}
		...
		>
			...
			<label class="mb-6 block">
				<span class="mb-1 block text-sm font-medium text-gray-700 dark:text-gray-300">Confirm password</span>
				<input
					type="password"
		...
		<p class="mt-4 text-center text-sm text-gray-600 dark:text-gray-400">
			Already have an account? <a href="/login" class="text-gray-900 underline dark:text-gray-100">Sign in</a>
		</p>
	{/if}
</div>
